...
void generate(unsigned int * seedp, double * p, unsigned int N, unsigned int min, unsigned int max)
{
    for (unsigned int i = 0; i < N; i++) {
        p[i] = (rand_r(seedp) % max) + min;
    }
}
...
int main(int argc, char * argv[])
{
    ...
    for (unsigned int i = 0; i < c_experiments; i++) {
        ...
        unsigned int seed = i;
        generate(&seed, m1, N, 1, c_a);
        //    puts("M1");
        //    print_array(m1, N);
        seed = i;
        generate(&seed, m2, N / 2, c_a, 10 * c_a);
        //    puts("M2");
        //    print_array(m2, N / 2);
        ...
        #pragma omp parallel for default(none) private(j) shared(m2, N) reduction(min:min_non_zero)
        for (j = 0; j < N / 2; j++) {
            if (m2[j] != 0) {
        ...
        #pragma omp parallel for default(none) private(j) shared(m2, N, min_non_zero) reduction(+:reduced_sum)
        for (j = 0; j < N / 2; j++) {
